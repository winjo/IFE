(function() {
	...
	first[0].addEventListener("click", function(e) {
		targetNode = e.target;

		if (targetNode.className.indexOf("active") < 0) {
			targetNode.className += " active";
		} else {
			targetNode.className = (" " + targetNode.className + " ").replace(" active ", " ");
		}
		...
		if (targetNode !== null) {
			tree.traverseSearch(function(node) {
				if (node.data === targetNode) {
					...
					var index = findIndex(node.parent.children, targetNode);
